...
int main(int argc, char **argv)
{
      ...
      pid_t pid; //descriptor del proceso hijo
/* 
...
for(;EVER;){
         ...
         pid = fork(); // creo un nuevo proceso hijo que va a atender las solicitudes

         if(!pid){ //si este es el proceso  hijo... 
            /*
...
void ISRsw( int sig ){
   int estado;
   pid_t pid;
   if( sig == SIGUSR1 ){ //señal del proceso hijo
      pid=wait( &estado );
      printf("SIGUSR1 recibida por el proceso %d\n",pid);
   }
